...
# Main Function for VedAI
def main():
    st.set_page_config(page_title="Ved AI 🛕", layout="wide")
    st.header("Learn with VedAI 🪷")
        ...
        if st.button("NEXT"):
            if validate_user_input(files):
                with st.spinner("Processing your Files..."):
                    ...
                    text_chunks = get_text_chunks(raw_text, file_names)
...
def get_text_chunks(text, file_names):
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=2048, chunk_overlap=512)
    chunks = []
    for file_name, chunk_text in zip(file_names, text_splitter.split_text(text)):
        chunks.append(f"{file_name}: {chunk_text}")
    return chunks
...
#     sol_model = ChatOllama(model="llama3")  
...
def user_input(user_question):
    if st.session_state.conversation:
        try:
            ...
            is_feedback = "@vedai" in user_question.lower()
            is_feedback = is_feedback or "@vedai" in user_question.upper()
